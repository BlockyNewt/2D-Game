...
void StatButton::setSettings(float radius, float x, float y, const sf::Color& fillColor, float outlineThickness, const sf::Color& outlineColor, STATNAME statName, int statIncrease, const ResourceFont& resourceFont)
{
	...
	*/
	...
	if (this->stat_Name_ != STATNAME::CLASSES)
	{
		description = "Increase stat by +" + std::to_string(this->stat_Increase_) + ".";

		this->t_A_.setSettings(resourceFont.getFont(FONT_TYPE::ARIAL), 18, std::to_string(this->amount_), sf::Vector2f(this->getRightPosition(true, 10.f), this->getBottomPosition(false, 20.f)), true);
...
void StatButton::updatePollEvent(sf::Event& ev, std::map<std::string, int>& stats)
{
	/*
...
void StatButton::update(const sf::Vector2i& mousePositionWindow)
{
	/*
...
void StatButton::render(sf::RenderTarget& target)
{
...
const float StatButton::getLeftPosition(bool pOrM, float offset) const
{
	/*
...
const float StatButton::getRightPosition(bool pOrM, float offset) const
{
	/*
		pOrM STAND FOR "PLUS OR MINUS". TRUE FOR ADDING THE OFFSET OR MINUS FOR SUBTRACTING THE OFFSET
	*/
...
const float StatButton::getTopPosition(bool pOrM, float offset) const
{
	/*
		pOrM STAND FOR "PLUS OR MINUS". TRUE FOR ADDING THE OFFSET OR MINUS FOR SUBTRACTING THE OFFSET
	*/
...
const float StatButton::getBottomPosition(bool pOrM, float offset) const
{
	/*
		pOrM STAND FOR "PLUS OR MINUS". TRUE FOR ADDING THE OFFSET OR MINUS FOR SUBTRACTING THE OFFSET
	*/
