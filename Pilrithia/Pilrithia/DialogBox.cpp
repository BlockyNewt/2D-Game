...
void DialogBox::setSettings(float sizeX, float sizeY, float posX, float posY, const sf::Color& fillColor, float outlineThickness, const sf::Color& outlineColor, bool isVisible)
{
	this->dialog_Background_.setSettings(300.f, 200.f, posX, posY, fillColor, outlineThickness, outlineColor, true);

	this->dialog_Text_.setSettings("Font/arial.ttf", 18, "Hello from the npc!", sf::Vector2f(this->dialog_Background_.getLeftPosition(), this->dialog_Background_.getTopPosition(true, 10.f)), true);

	this->expand_Button_.setSettings(150.f, 40.f, dialog_Background_.getLeftPosition(false, 300.f / 2.f - 150.f), this->dialog_Background_.getBottomPosition(false, 45.f), sf::Color::Red, outlineThickness, outlineColor, true);
